	...
	, show_flag_(false)
{
	...
	line_edit_->setObjectName("multiselect_lineedit");
	search_bar_->setObjectName("multiselect_lineedit_qu");
...
{
	line_edit_->clear();
	list_widget_->model()->removeRows(1, list_widget_->model()->rowCount() - 1);
...
void MultiSelectComboBox::clearEditText()
{
	line_edit_->clear();
	ResetSelection();
}
...
void MultiSelectComboBox::SetSelectionModel(SelectionModel _model)
{
...
#if 1
	//鼠标移入移出信号
	if (watched == line_edit_ && event->type() == QEvent::HoverEnter)
	{
		mouseSignalChange(true);
	}
	if (watched == line_edit_ && event->type() == QEvent::HoverLeave)
	{
		mouseSignalChange(false);
	}
#endif
...
void MultiSelectComboBox::stateChange(int _row)
{
	...
	emit currentIndexChanged(selected_data);
}
